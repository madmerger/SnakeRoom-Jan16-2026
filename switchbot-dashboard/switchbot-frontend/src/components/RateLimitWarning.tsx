interface RateLimitWarningProps {
  backoffRemaining: number;
}

export default function RateLimitWarning({ backoffRemaining }: RateLimitWarningProps) {
  return (
    <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-5 mb-6 flex items-center gap-4">
      <div className="w-10 h-10 rounded-lg bg-amber-500/20 flex items-center justify-center flex-shrink-0">
        <svg className="w-5 h-5 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth={2}
            d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"
          />
        </svg>
      </div>
      <div>
        <p className="font-semibold text-amber-400">Rate Limited</p>
        <p className="text-sm text-slate-400">SwitchBot API rate limit reached. Retry in {backoffRemaining} seconds.</p>
      </div>
    </div>
  );
}
